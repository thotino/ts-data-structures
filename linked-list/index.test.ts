...
describe('[unit] LinkedList', () => {
  ...
  test('Should have elements', () => {
    ...
    expect([...list.nodes.map(node => node.value)]).toStrictEqual([3, 2, 1, 5, 4])
  })
  ...
  test('Should remove an element', () => {
    ...
    expect([...list.nodes.map(node => node.value)]).toStrictEqual([3, 1, 5, 4])
  })

  test('Should reverse the list', () => {
    list.reverse()
    expect([...list.nodes.map(node => node.value)]).toStrictEqual([4, 5, 1, 3])
  })
